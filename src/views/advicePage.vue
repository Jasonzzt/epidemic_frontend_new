<template>
  ...
  <div class="com-page2">
    ...
    <el-container>
      ...
      <el-main style="font-size: 22px; font-family: 宋体">请选择目的地所在城市: <br>
        <div class="block">
...
<!--              请输入手机号：-->
            <span class="demonstration" style="margin-left: 50px;">手机号</span>
              <el-input
                  type="text"
                  style="width: 180px"
                  placeholder="请输入内容"
...
<!--            </div>-->
        ...
        <el-row :gutter="20" style="margin-top: 10px;margin-top: 100px">
          <el-col :span="10" style="font-size: 19px;margin-left: 90px">
            <dv-border-box-1><div class="contentin bgc-size flex" >目的地迁入排名
...
export default {
  ...
  methods: {
    ...
    search(){
      ...
      this.$axios.post('http://116.62.153.183/getCityEpidemicDataById',{"cityId":value1[0]},config).then(res=>{

      })
      this.$axios.post('http://116.62.153.183/getPopulationInByCityId',{"cityId":value1[0]},config).then(res=>{

      })
      this.$axios.post('http://116.62.153.183/getPolicy',{"cityId":value1[0]},config).then(res=>{

      })
      this.$axios.post('http://116.62.153.183/getSuggestion',{"cityId":value1[0]},config).then(res=>{
